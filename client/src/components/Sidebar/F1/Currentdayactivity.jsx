...
export default function Currentdayactivity(){
    ...
    return (state.addCurrentDayActivity ? 
    <div className={`addActivityBar ${state.fthState? "addActivityBar1" : "addActivityBar2"}`}>
        <Currentactivitysetup></Currentactivitysetup>
    </div> : 
    <div className={`currentActivities ${state.fthState? "scheduleDisclaimer1" : "scheduleDisclaimer2"}`}>
        <button className="addCurrentActivityButton" onClick={()=>{takeAction({type:"changeCurrentDayState", payload:true})}}>Add Current Day Activities</button>
    </div>
    )
}
